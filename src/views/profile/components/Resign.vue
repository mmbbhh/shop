    ...
    methods: {
      tologin() {
        if (this.user.length == 0 || this.password.length == 0 || this.tele.length == 0 || this.confirm.length == 0) {
          ...
          if (this.male == false) {
            const user = new login_resign.User(this.user, this.password, '女', this.tele)
            login_resign.resign(user).then(res => {
              if (res.data.state == 1) {
            ...
            const user = new login_resign.User(this.user, this.password, '男', this.tele)
            login_resign.resign(user).then(res => {
              if (res.data.state == 1) {
